...
class GymWrapper(object):
    ...
    def reset(self, epoch):
        ...
        # 把TFJ环境中的观测obs（tuple(tuple（array)）转变为SCHEDNET中需要的初级obs（list）
        obs = self.change_obs(obs)
        # obs = np.squeeze([np.concatenate([y.reshape(1,-1) for y in x],axis=1) for x in obs])
        return obs
    ...
    def get_stat(self):
        if hasattr(self.env, 'stat'):
            ...
            return dict()
    def change_obs(self, data):
        data_ = []
        for x in data:
            temp = [x[0],x[1]]
            temp+= x[2].flatten().tolist()  # 用➕合并两个list
            data_.append(temp)
        return data_

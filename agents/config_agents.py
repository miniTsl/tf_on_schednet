...
def config_agent(_flags):
    ...
    flags.DEFINE_integer("epoch_size", 40000, "totla steps for each epoch")
    
    flags.DEFINE_integer("update_gap", 100, "steps between updates")
    
    # schednet中的参数
    flags.DEFINE_string("agent", "schednet", "Agent")
    flags.DEFINE_integer("training_step", 80000000, "Training time step")
    flags.DEFINE_integer("testing_step", 40000, "Testing time step")
    ...
    flags.DEFINE_integer("eval_step", 400000, "Number of steps before evaluating = 10 epochs")
